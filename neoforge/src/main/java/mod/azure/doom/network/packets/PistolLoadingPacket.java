...
public class PistolLoadingPacket {

	public int slot;

	public PistolLoadingPacket(int slot) {
		this.slot = slot;
	}

	public PistolLoadingPacket(final FriendlyByteBuf packetBuffer) {
	...
	public static void handle(PistolLoadingPacket packet, Supplier<NetworkEvent.Context> ctx) {
		ctx.get().enqueueWork(() -> {
			...
			if (handler instanceof ServerGamePacketListenerImpl) {
				...
				PistolItem.reload(playerEntity, InteractionHand.MAIN_HAND);
			}
		});
