...
public class DPlasmaLoadingPacket {

	public int slot;

	public DPlasmaLoadingPacket(int slot) {
		this.slot = slot;
	}

	public DPlasmaLoadingPacket(final FriendlyByteBuf packetBuffer) {
	...
	public static void handle(DPlasmaLoadingPacket packet, Supplier<NetworkEvent.Context> ctx) {
		ctx.get().enqueueWork(() -> {
			...
			if (handler instanceof ServerGamePacketListenerImpl) {
				...
				DPlasmaRifle.reload(playerEntity, InteractionHand.MAIN_HAND);
			}
		});
