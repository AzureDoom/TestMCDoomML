...
public class ArchvileEntity extends DemonEntity implements SmartBrainOwner<ArchvileEntity> {
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH, MCDoom.config.archvile_health).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
	...
	public void spawnFlames(double x, double z, double maxY, double y, float yaw, int warmup) {
		...
		if (flag) {
			final var fang = new DoomFireEntity(level(), x, blockpos.getY() + d0, z, yaw, 1, this,MCDoom.config.archvile_ranged_damage);
