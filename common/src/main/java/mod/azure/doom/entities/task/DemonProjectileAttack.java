    ...
    protected void doDelayedAction(E entity) {
        ...
        if (entity instanceof ChaingunnerEntity || entity instanceof ShotgunguyEntity || entity instanceof ZombiemanEntity) {
            ...
            entity.level().getNearbyEntities(LivingEntity.class, checkBlocking, entity, aabb).stream().findFirst().ifPresent(target -> {
                ...
                target.hurt(entity.damageSources().mobAttack(entity), entity instanceof ChaingunnerEntity ? MCDoom.config.chaingun_bullet_damage : damage);
            });
        }
...
//			archvileEntity.teleportRandomly();
        ...
        if (entity instanceof MancubusEntity mancubusEntity) {
            if (mancubusEntity.distanceTo(target) < 8.0D && mancubusEntity.distanceTo(target) > 3.0D) // shoot flames
                mancubusEntity.shootMancubus(mancubusEntity, MCDoom.config.mancubus_ranged_damage);
            else // shoot ball
                mancubusEntity.shootBaron(mancubusEntity, MCDoom.config.mancubus_ranged_damage, 0, 0, 0);
        }

        if (entity instanceof MotherDemonEntity motherdemonEntity) {
            final var tentacleEntity = Services.ENTITIES_HELPER.getTentacleEntity().create(entity.level());
            ...
            if (entity.getHealth() <= entity.getMaxHealth() * 0.50) { // summon flames
                ...
                entity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + (motherdemonEntity.getDeathState() == 1 ? MCDoom.config.motherdemon_phaseone_damage_boos : 0), 0);
                entity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + (motherdemonEntity.getDeathState() == 1 ? MCDoom.config.motherdemon_phaseone_damage_boos : 0), 3);
                entity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + (motherdemonEntity.getDeathState() == 1 ? MCDoom.config.motherdemon_phaseone_damage_boos : 0), -3);
            }
        }
        ...
        if (entity instanceof DoomHunterEntity doomHunterEntity) {
            ...
            if (doomHunterEntity.getDeathState() == 0) {
                doomHunterEntity.shootRocket(this.target, MCDoom.config.doomhunter_ranged_damage + (doomHunterEntity.getDeathState() == 1 ? MCDoom.config.doomhunter_extra_phase_two_damage : 0));
            }
        }

        if (entity instanceof PainEntity painEntity) {
            entity.playSound(mod.azure.doom.platform.Services.SOUNDS_HELPER.getPAIN_HURT(), 1.0F, 1.0F);
            var lost_soul = Services.ENTITIES_HELPER.getLostSoulEntity().create(entity.level());
            ...
            if (painEntity.getVariant() == 2) { // if doom 64, summon another
                var lost_soul1 = Services.ENTITIES_HELPER.getLostSoulEntity().create(entity.level());
        ...
        if (entity instanceof ArchMakyrEntity archMakyrEntity) {
            if (entity.getRandom().nextInt(0, 4) == 1) // shoot fireball
                entity.shootFireball(entity, MCDoom.config.archmaykr_ranged_damage
                        + (archMakyrEntity.getDeathState() == 1 ? MCDoom.config.archmaykr_phaseone_damage_boost : archMakyrEntity.getDeathState() == 2 ? MCDoom.config.archmaykr_phasetwo_damage_boost : archMakyrEntity.getDeathState() == 3 ? MCDoom.config.archmaykr_phasethree_damage_boost : archMakyrEntity.getDeathState() == 4 ? MCDoom.config.archmaykr_phasefour_damage_boost : 0), 0);
            else { // summon flames
                for (var i = 1; i < 5; ++i) {
        ...
        if (entity instanceof IconofsinEntity iconEntity) {
            ...
            if (randomAttack == 1) {// Summon Fire on target
                ...
                entity.shootFireball(entity, MCDoom.config.icon_melee_damage + (iconEntity.getDeathState() == 1 ? MCDoom.config.icon_phaseone_damage_boos : 0), 0);
                if (entity.getHealth() < (entity.getMaxHealth() * 0.50))
        ...
        if (entity instanceof GladiatorEntity gladiatorEntity) {
            if (gladiatorEntity.getDeathState() == 0) {
                ...
                gladiatorEntity.shootFireball(this.target, MCDoom.config.gladiator_ranged_damage + (gladiatorEntity.getDeathState() == 1 ? MCDoom.config.gladiator_phaseone_damage_boost : 0), 0);
            } else
