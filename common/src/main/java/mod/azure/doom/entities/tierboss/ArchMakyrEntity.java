...
public class ArchMakyrEntity extends DemonEntity implements SmartBrainOwner<ArchMakyrEntity> {
    ...
    public BrainActivityGroup<ArchMakyrEntity> getFightTasks() {
        return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive()), new SetWalkTargetToAttackTarget<>().speedMod((owner, target) -> 2.05F), new DemonProjectileAttack<>(7).attackInterval(mob -> 240));
    }
    ...
    public static Builder createMobAttributes() {
        return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MOVEMENT_SPEED, 0.55D).add(Attributes.FLYING_SPEED, 0.25D).add(Attributes.MAX_HEALTH, MCDoom.config.archmaykr_health).add(Attributes.KNOCKBACK_RESISTANCE, 0.6f).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.ATTACK_KNOCKBACK, 1.0D);
    }
    ...
    public void spawnFlames(double x, double z, double maxY, double y, float yaw, int warmup) {
        ...
        if (flag) {
            final var fire = new DoomFireEntity(level(), x, blockpos.getY() + d0, z, yaw, 1, this, MCDoom.config.archmaykr_ranged_damage + (entityData.get(DEATH_STATE) == 1 ? MCDoom.config.archmaykr_phaseone_damage_boost : entityData.get(DEATH_STATE) == 2 ? MCDoom.config.archmaykr_phasetwo_damage_boost : entityData.get(DEATH_STATE) == 3 ? MCDoom.config.archmaykr_phasethree_damage_boost : entityData.get(DEATH_STATE) == 4 ? MCDoom.config.archmaykr_phasefour_damage_boost : 0));
