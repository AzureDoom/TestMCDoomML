...
public class PinkyEntity extends DemonEntity implements SmartBrainOwner<PinkyEntity> {
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.6f).add(Attributes.MAX_HEALTH, MCDoom.config.pinky_health).add(Attributes.ATTACK_DAMAGE, MCDoom.config.pinky_melee_damage).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 5.0D);
	}
