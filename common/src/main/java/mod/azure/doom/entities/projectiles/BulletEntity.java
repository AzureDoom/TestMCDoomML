    ...
    public void tick() {
        ...
        if (this.level().isClientSide()) {
            if (this.useParticle() == 1)
                this.level().addParticle(mod.azure.doom.platform.Services.PARTICLES_HELPER.getPISTOL(), true, this.getX() + (this.random.nextDouble()) * (double) this.getBbWidth() * 0.5D, this.getY(), this.getZ() + (this.random.nextDouble()) * (double) this.getBbWidth() * 0.5D, 0, 0, 0);
