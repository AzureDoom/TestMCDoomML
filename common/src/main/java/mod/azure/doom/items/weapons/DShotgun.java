...
public class DShotgun extends DoomBaseItem {
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof DShotgun) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0 && user.getInventory().countItem(Services.ITEMS_HELPER.getShells()) > 0) {
                ...
                user.level().playSound((Player) null, user.getX(), user.getY(), user.getZ(), Services.SOUNDS_HELPER.getSHOTGUNRELOAD(), SoundSource.PLAYERS, 1.00F, 1.0F);
            }
        }
    }
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private DSGRender renderer = null;
