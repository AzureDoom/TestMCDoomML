    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof SuperShotgun) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0 && user.getInventory().countItem(Services.ITEMS_HELPER.getShells()) > 0) {
                ...
                user.level().playSound((Player) null, user.getX(), user.getY(), user.getZ(), Services.SOUNDS_HELPER.getSHOTGUNRELOAD(), SoundSource.PLAYERS, 1.0F, 1.5F);
            }
        }
    }
        ...
        if (stack.getDamageValue() < stack.getMaxDamage() - 2) {
            if (!world.isClientSide() && stack.getItem() instanceof SuperShotgun) {
                ...
                if (!((PlayerProperties) player).hasMeatHook()) {
                    ...
                    hookshot.setProperties(stack, MCDoom.config.max_meathook_distance, 10, player.getXRot(), player.getYRot(), 0f, 1.5f * (10 / 10));
