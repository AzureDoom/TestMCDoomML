...
public class ChainsawAnimated extends BaseSwordItem implements GeoItem {
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slot, boolean selected) {
        ...
        if (selected && stack.getMaxDamage() < stack.getMaxDamage() - 1)
            level.playSound(null, user.getX(), user.getY(), user.getZ(), Services.SOUNDS_HELPER.getCHAINSAW_IDLE(), SoundSource.PLAYERS, 0.05F, 1.0F / (level.random.nextFloat() * 0.4F + 1.2F) + 0.25F * 0.5F);
    }
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private ChainsawRender renderer = null;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (renderer == null)
                    return new ChainsawRender();
